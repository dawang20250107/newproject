...
function onNavClick() { emit('close-mobile') }
</script>
...
<template>
  <nav :class="['sidebar', collapsed ? 'collapsed' : '', mobileOpen ? 'mobile-open' : '']"
    ...
    <div class="sidebar-brand">
      <div class="brand-mark">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
          ...
          <path d="M7 14l3-3 2 2 4-5" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
          <defs>
      ...
      <Transition name="label-fade">
        <span v-if="!effectiveCollapsed" class="brand-name">财务分析系统</span>
      </Transition>
    </div>
    ...
    <div class="nav-links">
      <!-- 财务报表 -->
      <router-link v-if="auth.canPage('report')" to="/report" class="nav-item" :class="{ active: route.path === '/report' }" @click="onNavClick">
        <span class="nav-icon">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z"/><path d="M14 2v6h6M8 13h8M8 17h5"/>
          </svg>
        </span>
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-label">财务报表</span></Transition>
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-arrow">
      ...
      <!-- 数据加工 -->
      <router-link v-if="auth.canPage('data')" to="/data" class="nav-item" :class="{ active: route.path === '/data' }" @click="onNavClick">
        <span class="nav-icon">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <!-- 图表分析 -->
      <router-link v-if="auth.canPage('charts')" to="/charts" class="nav-item" :class="{ active: route.path === '/charts' }" @click="onNavClick">
        <span class="nav-icon">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <!-- 系统设置 -->
      <router-link v-if="auth.isSuperAdmin" to="/settings" class="nav-item" :class="{ active: route.path === '/settings' }" @click="onNavClick">
        <span class="nav-icon">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-label">系统设置</span></Transition>
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-arrow">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M9 18l6-6-6-6"/></svg>
        </span></Transition>
      </router-link>
      ...
      <router-link v-if="auth.isSuperAdmin" to="/users" class="nav-item" :class="{ active: route.path === '/users' }" @click="onNavClick">
        ...
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-label">用户管理</span></Transition>
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-arrow">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M9 18l6-6-6-6"/></svg>
        </span></Transition>
      </router-link>
      ...
      <router-link v-if="auth.isSuperAdmin" to="/permissions" class="nav-item" :class="{ active: route.path === '/permissions' }" @click="onNavClick">
        ...
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-label">权限配置</span></Transition>
        <Transition name="label-fade"><span v-if="!effectiveCollapsed" class="nav-arrow">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M9 18l6-6-6-6"/></svg>
        </span></Transition>
      </router-link>
...
.brand-mark {
  ...
  flex-shrink: 0; box-shadow: 0 0 18px rgba(201, 99, 66, 0.22);
}
...
.nav-item {
  display: flex; align-items: center; gap: 0; padding: 0; height: 44px;
  border-radius: 10px; color: rgba(196, 168, 152, 0.75);
...
.nav-item.active {
  background: rgba(201, 99, 66, 0.2); color: #fff;
...
.nav-icon { flex-shrink: 0; display: flex; align-items: center; justify-content: center; width: 46px; height: 44px; }
...
.sidebar-footer {
  padding: 14px 11px 20px; border-top: 1px solid rgba(255, 255, 255, 0.07);
...
.collapse-btn {
  ...
  background: rgba(45, 24, 16, 0.92); border: 1px solid rgba(255,255,255,0.14);
  color: rgba(196, 168, 152, 0.7);
  display: flex; align-items: center; justify-content: center;
  transition: all 0.2s; z-index: 110; cursor: pointer;
...
@media (max-width: 768px) {
  .sidebar {
    width: var(--nav-w); transform: translateX(-100%);
