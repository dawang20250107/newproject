...
function validateRegister() {
  ...
  if (!selectedDepts.value.length) return '请至少选择一个所属事业部'
  return ''
}
      ...
      <div class="login-brand">
        <div class="brand-icon-lg">
          <svg width="52" height="52" viewBox="0 0 60 60" fill="none">
            ...
            <path d="M18 36l8-8 6 6 10-12" stroke="white" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
            <defs>
        ...
        <h1 class="brand-title">财务分析系统</h1>
        <p class="brand-sub">洞察 · 决策 · 增长</p>
        <div class="brand-features">
          <div class="feature"><span class="f-dot"></span>三级科目专用报表</div>
          <div class="feature"><span class="f-dot"></span>因素瀑布 · 走势分析</div>
          <div class="feature"><span class="f-dot"></span>多事业部权限管控</div>
        </div>
      </div>
      ...
      <div class="login-card">
        <template v-if="mode === 'pending'">
          <div v-if="pendingState === 'waiting'" class="pending-state">
            ...
            <button class="btn btn-ghost mt" style="width:100%;justify-content:center" @click="switchMode('login')">返回登录</button>
          </div>

          <div v-else-if="pendingState === 'approved'" class="pending-state approved-state">
            <div class="confetti"><i v-for="n in 14" :key="n" :style="`--i:${n}`"></i></div>
          ...
          <div v-else class="pending-state">
            ...
            <button class="btn btn-primary mt" style="width:100%;justify-content:center" @click="switchMode('login')">返回登录</button>
          </div>
        </template>
        ...
        <template v-else>
          ...
          <div v-if="mode === 'register' && isFirstUser" class="alert alert-info" style="margin-bottom:16px">
            系统首位注册用户将自动成为「超级管理员」。
          </div>
          ...
          <template v-else>
            ...
            <div class="lf-group">
              ...
              <div class="radio-group">
                <label v-for="jt in JOB_OPTIONS" :key="jt.v" class="radio-item" :class="{ selected: jobTitle === jt.v }">
            ...
            <div class="lf-group">
              <label>所属事业部 <span class="req">*</span> <span class="hint-text">可多选</span></label>
...
.login-wrap {
  ...
  max-width: 960px; width: 95vw; border-radius: 26px; overflow: hidden;
...
@keyframes cardIn { from { opacity:0; transform:scale(0.94) translateY(20px); } to { opacity:1; transform:scale(1) translateY(0); } }
...
.brand-icon-lg {
  ...
  box-shadow: 0 0 28px rgba(201,99,66,0.25); margin-bottom: 4px;
...
.login-card {
  flex: 1; background: rgba(255,252,248,0.78);
  backdrop-filter: blur(28px) saturate(180%); -webkit-backdrop-filter: blur(28px) saturate(180%);
  padding: 48px 44px; display: flex; flex-direction: column; justify-content: center;
}

.mode-tabs { display: flex; gap: 0; margin-bottom: 28px; background: rgba(0,0,0,0.05); border-radius: 12px; padding: 4px; width: fit-content; }
.mode-tab { padding: 8px 28px; border-radius: 8px; font-size: 14px; font-weight: 500; cursor: pointer; border: none; background: transparent; color: var(--muted); transition: all 0.22s; }
.mode-tab.active { background: rgba(255,252,248,0.9); color: var(--primary); font-weight: 700; box-shadow: 0 2px 10px rgba(100,60,30,0.12); }
...
.input-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: rgba(155,128,112,0.6); pointer-events: none; }
.has-icon { padding-left: 38px !important; }

.radio-group { display: flex; gap: 8px; flex-wrap: wrap; }
.radio-item { display: flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 20px; border: 1.5px solid var(--border); background: rgba(255,253,250,0.7); font-size: 13px; cursor: pointer; transition: all 0.18s; }
.radio-item input { display: none; }
.radio-item.selected { border-color: var(--primary); background: rgba(201,99,66,0.08); color: var(--primary); font-weight: 600; box-shadow: 0 0 0 2px rgba(201,99,66,0.12); }
...
.check-item { display: flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 20px; border: 1.5px solid var(--border); background: rgba(255,253,250,0.7); font-size: 12px; cursor: pointer; transition: all 0.18s; user-select: none; }
.check-item.selected { border-color: var(--primary); background: rgba(201,99,66,0.1); color: var(--primary); font-weight: 600; }
.check-box { width: 14px; height: 14px; border-radius: 4px; border: 1.5px solid currentColor; background: transparent; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: background 0.15s; }
.check-item.selected .check-box { background: var(--primary); border-color: var(--primary); }

.submit-btn { width: 100%; justify-content: center; padding: 12px; font-size: 15px; font-weight: 600; border-radius: 12px; margin-top: 8px; letter-spacing: 0.04em; }
.loading-dot { width: 14px; height: 14px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.4); border-top-color: white; animation: spin 0.7s linear infinite; }
...
.approve-loader .ring { position: absolute; inset: 0; border-radius: 50%; border: 3px solid rgba(201,99,66,0.15); border-top-color: var(--primary); animation: spin 1s linear infinite; }
.approve-loader .ring-icon { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-size: 30px; animation: float 2s ease-in-out infinite; }
.live-dot { width: 7px; height: 7px; border-radius: 50%; background: #2e7d32; display: inline-block; animation: livepulse 1.4s ease-in-out infinite; }
...
.approved-title { color: #2e7d32; font-size: 23px !important; animation: joyPop 0.55s cubic-bezier(0.34,1.7,0.5,1) both; }
...
.confetti i { position: absolute; top: 18%; left: 50%; width: 8px; height: 14px; border-radius: 2px; background: hsl(calc(var(--i) * 50), 85%, 60%); opacity: 0; animation: confettiBurst 1.5s calc(var(--i) * 0.04s) ease-out infinite; }
@keyframes confettiBurst {
  0%   { opacity: 0; transform: translate(0,0) rotate(0deg) scale(0.6); }
  15%  { opacity: 1; }
  100% { opacity: 0; transform: translate(calc((var(--i) - 7) * 26px), calc(120px + var(--i) * 6px)) rotate(calc(var(--i) * 90deg)) scale(1); }
}
