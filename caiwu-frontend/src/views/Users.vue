  ...
  try {
    ...
    for (const u of users.value) {
      if (!u.is_approved && u.role !== 'super_admin') {
        if (!(u.id in approveJob.value)) approveJob.value[u.id] = u.job_title || 'finance_bp'
...
async function reject(u) {
  if (!confirm(`确定拒绝「${u.name}」的注册申请？拒绝后该账号将被停用。`)) return
  try {
...
<template>
  <div>
    ...
    <div class="tabs">
      ...
      <button :class="['tab', tab === 'all' ? 'active' : '']" @click="tab = 'all'">已审批用户</button>
    </div>
    ...
    <div class="card">
      ...
      <template v-else-if="tab === 'pending'">
        <div v-if="!pendingUsers.length" class="empty"><div class="icon">✅</div>暂无待审批申请</div>
        <div v-else class="pending-list">
          <div v-for="u in pendingUsers" :key="u.id" class="pending-card">
              ...
              <div style="flex:1;min-width:0">
                ...
                <div class="approve-cfg">
                  ...
                  <div class="cfg-line" style="align-items:flex-start">
                    <span class="cfg-label" style="margin-top:5px">负责事业部</span>
                    <div class="dept-pick">
                      <label v-for="d in BUSINESS_UNITS" :key="d"
            ...
            <div class="pending-actions">
              <button class="btn btn-success btn-sm" :disabled="approveLoading[u.id]" @click="approve(u)">
      ...
      <template v-else>
        ...
        <div v-else class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>用户</th><th>手机号</th><th>角色</th><th>职务</th>
                <th>负责事业部</th><th>状态</th><th>创建时间</th><th>操作</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="u in activeUsers" :key="u.id">
                ...
                <td>
                  <span v-if="u.role === 'super_admin'" style="color:var(--muted);font-size:12px">全部</span>
    ...
    <div v-if="showEditModal" class="overlay" @click.self="showEditModal=false">
      <div class="modal" style="width:520px">
        ...
        <div v-if="editUser?.role !== 'super_admin'" class="form-group" style="margin-bottom:14px">
          <label>负责事业部</label>
...
.pending-badge {
  ...
  font-size: 13px; font-weight: 600; border: 1px solid rgba(245,127,23,0.25);
...
.pending-card {
  ...
  background: rgba(245,127,23,0.05); border: 1px solid rgba(245,127,23,0.15); transition: all 0.2s;
}
...
.pa-chip { background: rgba(21,101,192,0.1); color: #1565c0; border-radius: 10px; padding: 1px 7px; font-size: 11px; font-weight: 600; }
.pa-time { font-size: 11px; color: rgba(155,128,112,0.65); margin-top: 4px; }

.pending-actions { display: flex; flex-direction: column; align-items: stretch; gap: 8px; flex-shrink: 0; min-width: 96px; }
.role-select { width: auto; min-width: 110px; padding: 5px 8px; font-size: 12px; border-radius: 7px; }

.approve-cfg { margin-top: 10px; padding-top: 10px; border-top: 1px dashed rgba(245,127,23,0.25); display: flex; flex-direction: column; gap: 8px; }
.cfg-line { display: flex; align-items: center; gap: 10px; }
.cfg-label { font-size: 12px; color: var(--muted); width: 64px; flex-shrink: 0; }
.dept-pick { display: flex; flex-wrap: wrap; gap: 5px; }
.dept-mini { padding: 3px 9px; border-radius: 14px; font-size: 11px; cursor: pointer; border: 1.5px solid var(--border); background: rgba(255,253,250,0.7); transition: all 0.16s; user-select: none; }
.dept-mini.on { border-color: var(--primary); background: rgba(201,99,66,0.1); color: var(--primary); font-weight: 600; }
...
.radio-item { display: flex; align-items: center; gap: 6px; padding: 6px 13px; border-radius: 20px; border: 1.5px solid var(--border); background: rgba(255,253,250,0.7); font-size: 13px; cursor: pointer; transition: all 0.18s; }
.radio-item.selected { border-color: var(--primary); background: rgba(201,99,66,0.08); color: var(--primary); font-weight: 600; }

.check-group { display: flex; flex-wrap: wrap; gap: 7px; }
.check-item { display: flex; align-items: center; gap: 6px; padding: 5px 11px; border-radius: 20px; border: 1.5px solid var(--border); background: rgba(255,253,250,0.7); font-size: 12px; cursor: pointer; transition: all 0.18s; user-select: none; }
.check-item.selected { border-color: var(--primary); background: rgba(201,99,66,0.1); color: var(--primary); font-weight: 600; }
.check-box { width: 14px; height: 14px; border-radius: 4px; border: 1.5px solid currentColor; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: background 0.15s; }
