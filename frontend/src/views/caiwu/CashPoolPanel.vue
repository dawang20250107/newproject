...
async function load() {
  ...
  try {
    ...
    err.value = e?.msg || (typeof e === 'string' ? e.slice(0, 200) : '加载失败，请稍后重试；若持续失败请联系管理员')
    loading.value = false
    return
  }
  try {
          ...
          <div class="modal-box">
            ...
            <div class="modal-bd">
              <ul class="method-list">
                ...
                <li><b>现金流出</b> ＝ 实付分期 ＋ 预付款。<b>预付核销冲抵</b>不计现金流出——预付的现金在预付款项日期当天已作为「预付款」流出，核销只是把这笔预付资产结转到某张应付上，本身无新现金事件（与预收冲抵对称）。实付分期与预付冲抵是计划的两块互不重叠部分（已付＋冲抵＝已覆盖），故实付分期即本期真实付现。</li>
