...
// 含"行业/同行调研"的问题仅在联网检索开启时展示，否则替换为内部数据向的问题，
// 避免抛出助手当前无法回答（未联网）的建议。
const webResearch = computed(() => !!data.value?.web_research)
const SUGGESTIONS = computed(() => {
  const m = `${month.value}月`
  const w = webResearch.value
  const byBu = {
    '': [
      `生成${m}集团经营分析报告`,
      '哪个事业部在拖累集团利润？为什么？',
      `${m}回款和收入是否匹配？应收风险在哪？`,
      w ? '对比行业同行，我们的盈利水平处于什么位置？' : '各事业部毛利率排名与差距在哪？',
    ],
    '集团总部': [
      `总部${m}费用结构分析：哪些费用在涨？`,
      '总部费用分摊后各事业部的真实盈利如何？',
    ...
    '运输事业部': [
      ...
      w ? '调研一下公路货运行业最新运价与同行动态' : `运输${m}亏损与薄利项目有哪些？`,
    ],
    '劳务事业部': [
      `劳务${m}人效与用工成本分析`,
      '社保成本率的变化对劳务毛利影响多大？',
      `劳务${m}目标缺口多少？给出追赶方案`,
      w ? '灵活用工行业政策最近有什么新动向？' : `劳务${m}回款质量与应收风险`,
    ],
    '供应链事业部': [
      `供应链${m}经营分析：毛利与资金占用`,
      `供应链${m}回款与内部往来挂账情况`,
      w ? '供应链物流行业有什么趋势和机会？' : `供应链${m}亏损与薄利项目定位`,
      `生成供应链${m}经营分析报告`,
    ],
    '多式联运事业部': [
      `多式联运${m}经营分析与目标达成`,
      '多式联运的成本结构和毛利动因是什么？',
      `多式联运${m}回款与应收风险`,
      w ? '多式联运行业政策与市场有什么新变化？' : `多式联运${m}目标缺口与追赶方案`,
    ],
  }
