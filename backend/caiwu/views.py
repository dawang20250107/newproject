...
# 多段科目编码覆盖（优先于单段编码前缀）——仅 6602.99.03「集团管理费用」本科目
# （集团管理费分摊/收回）单列为 KXT 集团管理费用；其同级的 6602.99.01 培训费、
# 6602.99.02 会议费等属普通管理费用，应回落到 6602 前缀 → 管理费用，不计入集团管理费。
# （科目名称含「集团管理费用」者另由 _KD_NAME_L1 兜底覆盖。）
...
要求：专业、有数据支撑、有洞察，避免空话套话；分点清晰，关键指标对比用 Markdown 表格呈现（| 表头 | … | 配 | --- | 分隔行），适合事业部管理层决策参考，篇幅 600-1000 字。"""
    else:
...
要求：专业、有数据支撑、有洞察，避免空话套话；分点清晰，事业部横向对比/关键指标并列务必用 Markdown 表格呈现（| 表头 | … | 配 | --- | 分隔行），适合集团领导决策参考，篇幅 800-1200 字。"""
    return f"{header}\n\n{data}\n\n{body}"
